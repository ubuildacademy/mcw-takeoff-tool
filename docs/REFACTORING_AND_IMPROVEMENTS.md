...
- **Deploy fix (commit after b1881a92)** – Commit b1881a92 (lint cleanup) removed the **`SelectionBox`** export from `PDFViewer.types.ts`, which broke the build: `pdfViewerRenderers.ts` and `usePDFViewerInteractions.types.ts` import it. Restored **`export interface SelectionBox { x, y, width, height }`** in `PDFViewer.types.ts` so typecheck and Vercel deploy succeed. When committing further lint/cleanup work, ensure `PDFViewer.types.ts` is included and still exports `SelectionBox`.

...
| **Wand: furniture-aware fill** | Rooms with beds/rugs force many clicks (interior linework splits regions). Idea: morphological opening on the raster before fill — erode strokes thinner than N px so furniture outlines stop blocking while thick walls survive. |
...
| Item | When |
|------|------|
| **Distributed rate limiting** | Rate-limit counters moved from a per-process `Map` to the Redis the project already runs for BullMQ, behind a `RateLimitStore` interface that keeps an in-memory implementation for local development and falls back to it if Redis stops answering. Increment and expiry are one Lua step, since a counter that gets an `INCR` without a `PEXPIRE` never resets and would lock a client out for good. Verified against a real Redis and confirmed live in production via an `X-RateLimit-Store: redis` response header. **Note the item that prompted this was a misdiagnosis** — production runs one instance, and the split counters that looked like two replicas were one client arriving from two NAT addresses (item 27 in `docs/OPEN_ITEMS.md`). The change is correct and is what a second replica would need; it was not fixing a live defect. | 2026-08 |
| **Themed confirmation dialogs** | All 14 native `confirm()` boxes replaced with the app's own `ConfirmDialog`, which had existed unused since the dialog primitives landed. `hooks/useConfirm.tsx` gives a promise-based `await confirm({ title, description })` so no handler had to be split around the user's answer; the pending promise settles once whether the user confirms, cancels, escapes or clicks away. Destructive actions get the red confirm button. Prompt text rewritten per site — the native box has no title, so the old strings crammed question and consequence onto one line. The eight confirmations inside the Admin panel use `ui/confirm-inline.tsx` rather than a dialog, since the panel is itself a modal: the control arms in place and disarms on escape, a click away, or four seconds. | 2026-08 |
| **Codebase-wide simplification sweep** | Four area passes over ~93k lines (item 23 in `docs/OPEN_ITEMS.md`): frontend utils/services, server services/routes, Python, frontend components. 1,216 lines of code removed against 774 added, a net 442 down. Found and deleted one real defect — a failed-batch handler in the titleblock service always resolved to the first batch, so the wrong pages got "Unknown" placeholders — along with its dead caller. Consolidated six sets of copy-paste (Excel header styles, `escapeHtml`, multer upload plumbing, the Python subprocess runner, the pasted-measurement payload) and made five quadratic scans linear, including one that ran on every vertex drag. Python audited clean, no changes. `ci:local` held at 446 tests passed / 8 skipped and 70 frontend / 0 server lint warnings throughout. Large-component splits deliberately left under the when-touching rule. | 2026-08 |
...
| **Room proposals (whole-sheet)** — *removed* | ⌘K → "Propose rooms on this sheet": grid-seeded flood-fill sweep (`proposeRooms` in `floodFillRoom.ts`) finds every enclosed region once (shared visited mask, ~O(page)), filters leaks/dust, review dialog with computed areas → accepted rooms become measurements in the selected area/volume condition. Deterministic, no LLM. **Shelved 2026-07** (over-proposed non-room enclosures — legends, titleblock cells — with no footprint constraint) and **fully removed 2026-07** (`proposeRooms()`, `RoomProposalsDialog`, palette entry); the click-to-fill magic wand covers rooms reliably instead. Code recoverable from git history if revisited. | 2026-07 |
| **Schedule → takeoff (no LLM)** — *removed* | ⌘K → "Schedule → takeoff": drag a box around a door/window/fixture schedule → server `table_extract.py` reconstructs the table from line-grid geometry + exact vector text (word-alignment clustering fallback for borderless schedules) → review dialog with column mapping → each row becomes a count condition with QTY markers placed on its schedule row. **Cut 2026-07-17** and fully removed from the app: on the beta set the door schedule body was OUTLINED to vector paths by the CAD export (only ~1% of grid cells had text-layer content), so vector extraction could not read the schedules that mattered, and a raster-OCR fallback was not worth building for it. Code recoverable from the `archive/schedule-ocr-fallback` tag; the rotation fix it proved out lives on in `vector_callout_pass.py` and `pymupdf_text_extract.py`. Only revival path is a trusted schedule → auto-count placement. | 2026-07 |
| **Command palette (⌘K)** | Fuzzy jump to sheets (by number/name), activate conditions, run actions (calibrate, magic wand, fit, schedule takeoff). `src/components/CommandPalette.tsx`. | 2026-07 |

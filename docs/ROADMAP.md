...
| **Distributed rate limiting** | Item 27 in `docs/OPEN_ITEMS.md`. `rateLimitStore` is an in-memory `Map` per process and production runs two instances, so every configured limit is silently doubled — `strictRateLimit`'s 10 login attempts per 15 minutes is really about 20. Redis is already running in the same Railway project and already connected via `ioredis` in `queueService.ts`, so this is an `INCR`/`EXPIRE` swap, not new infrastructure. |

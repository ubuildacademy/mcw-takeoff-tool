...
| Item | When |
|------|------|
| **Distributed rate limiting** | Rate-limit counters moved from a per-process `Map` to the Redis the project already runs for BullMQ, behind a `RateLimitStore` interface that keeps an in-memory implementation for local development and falls back to it if Redis stops answering. Production runs two Railway instances, so every configured limit had been silently multiplied by two — `strictRateLimit` advertised 10 login attempts per 15 minutes and allowed about 20. Increment and expiry are one Lua step, because a counter that gets an `INCR` without a `PEXPIRE` never resets and would lock a client out for good. Verified against a real Redis, not just the test double. | 2026-08 |

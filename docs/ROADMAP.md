...
| Item | Notes |
|------|--------|
| **Codebase-wide simplification sweep** | Audit for vibe-coding accretion — duplicated logic, dead abstractions, over-parameterized helpers, stale comments. **Slices A (frontend utils/services), B (server services/routes) and C (Python) done 2026-08-20**, ~980 lines removed, one real defect found and deleted with its dead caller, Python clean. **Slice D (frontend components) open**: PDFViewer.tsx duplicates ~240 lines of the hooks extracted out of it. Detail in `docs/OPEN_ITEMS.md` item 23. |

/**
 * Titleblock extraction (runs in BullMQ worker).
 */
...
import { titleblockExtractionService } from './titleblockExtractionService';
...
export interface RunTitleblockExtractionParams {
  ...
  onProgress?: (p: TitleblockExtractionProgress) => void;
}

export async function runTitleblockExtraction(
  params: RunTitleblockExtractionParams
): Promise<{ success: true; results: TitleblockExtractionResultRow[] }> {
  const { projectId, documentIds, titleblockConfig, onProgress } = params;
  ...
  const results: TitleblockExtractionResultRow[] = [];
  const numDocs = documentIds.length;
  let completedPageSum = 0;
  ...
  for (let docIndex = 0; docIndex < documentIds.length; docIndex++) {
    const documentId = documentIds[docIndex];
      ...
      try {
        ...
        extractedSheets = await extractTitleblockWithLLM(
          ...
          (currentBatch: number, totalBatches: number, processedPages: number) => {
            ...
            const withinDoc = totalPages ? processedPages / totalPages : 0;
            const pct = Math.round(((docIndex + withinDoc) / numDocs) * 100);
            onProgress?.({
              percent: Math.min(99, pct),
              processedPages: completedPageSum + processedPages,
              totalPages: completedPageSum + totalPages,
            });
          }
        );
      ...
      completedPageSum += totalPages;
    }

  onProgress?.({ percent: 100, processedPages: completedPageSum, totalPages: completedPageSum });

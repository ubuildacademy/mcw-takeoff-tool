...
const PDFViewer: React.FC<PDFViewerProps> = ({ 
  ...
  useEffect(() => {
    if (pdfDocument && currentViewport) {
      ...
      // Render if we have measurements, annotations, or if we're in measuring/annotation/visual search mode
      // CRITICAL: Also render when in selection mode and markups exist - this ensures handlers are attached
      const shouldRender = hasAnyMeasurements || isMeasuring || isCalibrating || currentMeasurement.length > 0 || isAnnotating || localAnnotations.length > 0 || (visualSearchMode && isSelectingSymbol) || (!!titleblockSelectionMode && isSelectingSymbol) || (isSelectionMode && hasAnyMeasurements);
      if (shouldRender) {
  ...
  }, [renderTakeoffAnnotations, localTakeoffMeasurements, localAnnotations, currentProjectId, file?.id, isSelectionMode, isCalibrating, annotationTool, visualSearchMode, titleblockSelectionMode, isSelectingSymbol]);
  ...
  const renderPDFPage = useCallback(async (pageNum: number) => {
    ...
    try {
      ...
      // CRITICAL: Always render markups after clearing overlay to ensure they appear immediately
      ...
      // CRITICAL: Always render markups after PDF render completes
      // This ensures markups appear immediately when turning to a page
      // Use requestAnimationFrame to ensure DOM is ready after overlay was cleared
      requestAnimationFrame(() => {
        try {
          // Check both local measurements and store measurements
          ...
          if (process.env.NODE_ENV === 'development') {
            console.warn('Error rendering markups after PDF render:', error);
          }
        }
      });
  ...
  const handleClick = useCallback(async (event: React.MouseEvent<HTMLCanvasElement | SVGSVGElement>) => {
    ...
    if (visualSearchMode || !!titleblockSelectionMode) {
      ...
      // Visual search mode active but not selecting - prevent measurement clicks
      // User should wait for visual search to complete or exit visual search mode
      if (visualSearchMode) {
        return; // Don't allow manual measurements during visual search mode
      }
    }
  ...
  // This ref is used by click handlers to check current selection mode without closure issues
  useEffect(() => {
    ...
    if (svgOverlayRef.current) {
      ...
      if (allMarkups.length > 0) {
        ...
        requestAnimationFrame(() => {
          if (svgOverlayRef.current && isSelectionModeRef.current === isSelectionMode) {
  ...
  useEffect(() => {
    if (selectedConditionId) {
      ...
      if (condition) {
        ...
        // Set measurement type based on condition type

...
import {
  ...
  renderSVGHyperlinks,
  renderSVGCurrentCutoutCommitted,
  renderSVGCurrentCutoutEphemeral,
  renderSVGCrosshair,
  renderSVGMeasurement,
  renderSVGAnnotation,
  renderSVGCalibrationPointsCommitted,
...
import {
  ...
  clearEphemeralMarkupLayer,
  SVG_COMMITTED_GROUP_ID,
  SVG_EPHEMERAL_GROUP_ID,
  SVG_MOVE_DRAG_MEASUREMENT_WRAP_CLASS,
  SVG_MOVE_DRAG_ANNOTATION_WRAP_CLASS,
} from './pdf-viewer/pdfMarkupLayers';
...
const PDFViewer: React.FC<PDFViewerProps> = ({
  ...
  const [ocrHighlightBoxes, setOcrHighlightBoxes] = useState<
    Array<{
      index: number;
      text: string;
      confidence: number;
  ...
  /** Same pattern as isSelectionModeRef — blank-click deselect must not see stale isMeasuring before layout effect runs. */
  ...
  useEffect(() => {
    if (!effectiveProjectId || !file?.id || !ocrHighlightRequest) {
      setOcrHighlightBoxes([]);
      return;
    }

    const query = ocrHighlightRequest.query?.trim() ?? '';
    const sameDocument = ocrHighlightRequest.documentId === file.id;
    const samePage = ocrHighlightRequest.pageNumber === currentPage;
    if (!sameDocument || !samePage || query.length === 0) {
      setOcrHighlightBoxes([]);
      return;
    }
    ...
    (async () => {
      try {
        const response = await ocrApiService.getDocumentWordBoxes(
          file.id,
          effectiveProjectId,
          currentPage,
          query
        ...
        if (!cancelled) {
          setOcrHighlightBoxes(Array.isArray(response.boxes) ? response.boxes : []);
        }
      } catch (error) {
        if (!cancelled) {
          console.error('Failed to load OCR highlight boxes:', error);
          setOcrHighlightBoxes([]);
        }
      }
    })();
  ...
  }, [ocrHighlightRequest, currentPage, effectiveProjectId, file?.id]);
  ...
  const paintEphemeralMarkupLayer = useCallback(() => {
    ...
    if (isMeasuring && pageNum === currentPage) {
      const hasMeasurementPoints =
        (measurementType === 'linear' && isContinuousDrawing && activePoints.length > 0) ||
        (measurementType !== 'linear' && currentMeasurement.length > 0) ||
        (measurementType === 'linear' && !isContinuousDrawing && currentMeasurement.length > 0);
      if (hasMeasurementPoints && measurementType !== 'count') {
        renderSVGCurrentMeasurementEphemeral(ephemeral, viewport, {
          currentPage,
          measurementType,
          isContinuousDrawing,
          activePoints,
    ...
    if (cutoutMode && currentCutout.length > 0 && pageNum === currentPage) {
      renderSVGCurrentCutoutEphemeral(ephemeral, viewport, currentCutout, mousePosition);
    }

    if (isCalibrating && calibrationPoints.length > 0 && pageNum === currentPage) {
      renderSVGCalibrationPointsEphemeral(ephemeral, {
        calibrationPoints,
        viewport,
        mousePosition,
        isOrthoSnapping,
    ...
    if (annotationTool && pageNum === currentPage) {
      renderSVGCurrentAnnotation(ephemeralSvg, viewport, {
        annotationTool,
        currentAnnotation,
        mousePosition,
        annotationColor,
    ...
    if (
      ...
      renderSVGCrosshair(ephemeralSvg, mousePosition, viewport, isCalibrating, {
    ...
    if (showRunningLength && isContinuousDrawing && activePoints.length > 0 && pageNum === currentPage) {
      renderRunningLengthDisplay(ephemeralSvg, viewport, {
        runningLength,
        conditionColor: getSelectedCondition()?.color || '#000000',
  ...
  const scheduleEphemeralPaint = useCallback(() => {
    if (ephemeralRafRef.current != null) return;
    ephemeralRafRef.current = requestAnimationFrame(() => {
      ephemeralRafRef.current = null;
  ...
  useEffect(() => {
    if (localTakeoffMeasurements.length > 0 && pdfDocument && currentViewport) {
  ...
  const renderTakeoffAnnotations = useCallback((pageNum: number, viewport: PageViewport, page?: PDFPageProxy) => {
    ...
    const hasAnyMarkups = hasLocalMeasurements || hasAnnotations || hasHyperlinks || hasOcrHighlights;
    ...
    const { committed } = ensureMarkupLayerGroups(svgOverlay);
    const committedSvg = committed as unknown as SVGSVGElement;
    clearCommittedMarkupLayer(committed);
    ...
    committed.appendChild(hitArea);

    // OCR highlight boxes (above hit area, below interactive markups)
    if (ocrHighlightBoxes.length > 0) {
      const rot = viewState.rotation || 0;
      for (const box of ocrHighlightBoxes) {
        const corners = [
          baseNormToViewportPixels(box.bbox.x, box.bbox.y, { width: pixelW, height: pixelH }, rot),
          baseNormToViewportPixels(box.bbox.x + box.bbox.width, box.bbox.y, { width: pixelW, height: pixelH }, rot),
          baseNormToViewportPixels(box.bbox.x + box.bbox.width, box.bbox.y + box.bbox.height, { width: pixelW, height: pixelH }, rot),
          baseNormToViewportPixels(box.bbox.x, box.bbox.y + box.bbox.height, { width: pixelW, height: pixelH }, rot),
        ];
        ...
        rect.setAttribute('width', Math.max(0, xMax - xMin).toString());
        rect.setAttribute('height', Math.max(0, yMax - yMin).toString());
        rect.setAttribute('fill', 'rgba(250, 204, 21, 0.35)');
        rect.setAttribute('stroke', 'rgba(202, 138, 4, 0.7)');
        rect.setAttribute('stroke-width', '1');
        rect.setAttribute('vector-effect', 'non-scaling-stroke');
        rect.setAttribute('pointer-events', 'none');
        rect.setAttribute('data-ocr-highlight', 'true');
        committed.appendChild(rect);
      }
    }

    // Render hyperlinks (clickable regions) - above hit area
    if (pageHyperlinks.length > 0) {
      renderSVGHyperlinks(committedSvg, pageHyperlinks, viewport);
    }

    // Render measurements for this page (move-drag: wrap in <g class="...">; translate updated in effect, not here)
    const measurementIdsMoving =
      measurementMoveIds.length > 0
        ? measurementMoveIds
        : measurementMoveId
          ? [measurementMoveId]
          : [];
    pageMeasurements.forEach((measurement) => {
      const isMoving = measurementIdsMoving.includes(measurement.id);
      const targetParent = isMoving
        ? (() => {
            const wrap = document.createElementNS('http://www.w3.org/2000/svg', 'g');
            wrap.setAttribute('class', SVG_MOVE_DRAG_MEASUREMENT_WRAP_CLASS);
            committed.appendChild(wrap);
            return wrap;
          })()
        : committedSvg;
      renderSVGMeasurement(targetParent as unknown as SVGSVGElement, measurement, viewport, page, {
    ...
    if (measurementDebug && measurementDebug.page === pageNum) {
      ...
      committed.appendChild(t3);
    }

    // Draw current measurement — committed geometry only (dashed preview + crosshair in paintEphemeralMarkupLayer)
    if (isMeasuring && pageNum === currentPage) {
      const hasMeasurementPoints =
        (measurementType === 'linear' && isContinuousDrawing && activePoints.length > 0) ||
        (measurementType !== 'linear' && currentMeasurement.length > 0) ||
      ...
      if (hasMeasurementPoints && measurementType !== 'count') {
        renderSVGCurrentMeasurementCommitted(committed, viewport, {
          currentPage,
          measurementType,
          isContinuousDrawing,
          activePoints,
    ...
    localAnnotations.forEach((annotation) => {
      ...
      const isMoving = annotationIdsMoving.includes(annotation.id);
      const targetParent = isMoving
        ? (() => {
            const wrap = document.createElementNS('http://www.w3.org/2000/svg', 'g');
            wrap.setAttribute('class', SVG_MOVE_DRAG_ANNOTATION_WRAP_CLASS);
            committed.appendChild(wrap);
            return wrap;
          })()
        : committedSvg;
      renderSVGAnnotation(targetParent as unknown as SVGSVGElement, annotation, viewport, {
        rotation: viewState.rotation || 0,
        selectedMarkupIds,
        selectionMode: isSelectionMode,
      });
    ...
    if (isCalibrating && calibrationPoints.length > 0 && pageNum === currentPage) {
      renderSVGCalibrationPointsCommitted(committed, {
        calibrationPoints,
        viewport,
        isOrthoSnapping,
        applyOrthoSnapping,
  ...
  useEffect(() => {
    // Do not gate on isRenderingRef — during/after PDF canvas render the overlay was cleared; we must be able
    // to redraw markups/hyperlinks when the ref flips false, or navigation leaves an empty overlay and stuck view.
    if (pdfDocument && currentViewport) {
      ...
      // Include sheet hyperlinks for this file; otherwise we clear the overlay and break PDF/overlay sync
      const hasOcrHighlights = ocrHighlightBoxes.length > 0;
      const hasMarkups = hasLocalMarkups || hasStoreMarkups || hyperlinkCountForFile > 0 || hasOcrHighlights;
  ...
  useEffect(() => {
    ...
    const hasMarkups = localTakeoffMeasurements.length > 0 || localAnnotations.length > 0 || hyperlinkCountForFile > 0 || ocrHighlightBoxes.length > 0;
    if (hasMarkups) {
      renderMarkupsWithPointerEvents(currentPage, freshViewport, pdfPageRef.current ?? undefined);
    }
  }, [pdfDocument, viewState.scale, viewState.rotation, currentPage, localTakeoffMeasurements, localAnnotations, renderMarkupsWithPointerEvents, rendersBlocked, hyperlinkCountForFile, ocrHighlightBoxes]);
  ...
  }, [showMeasurementLabels, showRunningLength, hyperlinkCountForFile, ocrHighlightBoxes]);
  ...
  const onPageShown = useCallback((pageNum: number, viewport: PageViewport) => {
    ...
    // Hit-area is created inside `#pdf-markup-committed` by renderTakeoffAnnotations (avoids duplicate rects on the SVG root).
  ...
  }, [renderMarkupsWithPointerEvents, localTakeoffMeasurements, localAnnotations, effectiveProjectId, file.id, isSelectionMode, isCalibrating, isMeasuring, isAnnotating, isDrawingBoxSelection, annotationTool, hyperlinkMode, ocrHighlightBoxes]);
  ...
  const createRubberBandElement = useCallback(() => {
    ...
    const svgOverlay = svgOverlayRef.current;
    const { ephemeral } = ensureMarkupLayerGroups(svgOverlay);
    ...
    if (existingRubberBand && ephemeral.contains(existingRubberBand)) {
      ephemeral.removeChild(existingRubberBand);
    }
  ...
  useEffect(() => {
    if (pdfDocument && currentViewport && pdfPageRef.current) {
      // Call onPageShown to initialize overlay (must run even while a PDF render is in flight so overlay recovers)
  ...
  useLayoutEffect(() => {
    ...
    if (selectedConditionId) {
      ...
      prevSelectedConditionIdForModeRef.current = null;
      setIsMeasuring(false);
      setIsSelectionMode(true);
      setCurrentMeasurement([]);
      setMousePosition(null);
  ...
  // Space from workspace: leave plan-only markup selection and enter draw mode for the selected condition.
  useLayoutEffect(() => {

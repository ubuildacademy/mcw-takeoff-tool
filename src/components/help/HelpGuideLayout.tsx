...
import { HELP_GUIDE_SLUGS, HELP_GUIDE_TITLES } from '../../content/helpContent';
...
export function HelpGuideLayout({ title, currentSlug, children, aside, actions }: HelpGuideLayoutProps) {
  const otherSlugs = currentSlug
    ? HELP_GUIDE_SLUGS.filter((slug) => slug !== currentSlug)
    : [];
            ...
            <nav className="flex items-center gap-2 text-sm shrink-0">
              {otherSlugs.map((slug) => (
                <span key={slug} className="flex items-center gap-2">
                  <Link to={`/help/${slug}`} className="text-primary hover:underline whitespace-nowrap">
                    {HELP_GUIDE_TITLES[slug]}
                  </Link>

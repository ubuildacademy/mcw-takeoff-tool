...
>(({ className, children, showCloseButton = true, size, style, ...props }, ref) => {
  ...
  const keyboardStyle: React.CSSProperties =
    keyboardHeight > 0 ? { top: `calc(50% - ${keyboardHeight / 2}px)` } : {};
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ref={ref}
        className={cn(
          // overflow-x-hidden + min-w-0 are the design rule, not a one-off: a dialog must
          // never need horizontal scroll (Jeff, 2026-08-10) — content that would overflow
          // wraps or truncates instead. min-w-0 defeats grid/flex's default "never shrink
          // below intrinsic content width" so a long unbroken string can't blow out the box.
          "fixed left-[50%] top-[50%] z-50 grid w-full min-w-0 max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 overflow-x-hidden border bg-background p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] sm:rounded-lg",
